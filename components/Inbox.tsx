...
import { Search, Send, MessageSquare, Mail, MessageCircle, MoreVertical, Paperclip, Loader2, Sparkles, CheckCircle2, ArrowLeft } from 'lucide-react';
...
export const Inbox: React.FC<InboxProps> = ({ tenantId }) => {
    ...
    // If no thread is selected, select the first one on large screens only, 
    // but on mobile start with list view (activeThreadId = null)
    const [activeThreadId, setActiveThreadId] = useState<string | null>(null);
    
    // Auto-select first thread on desktop on mount if none selected
    useEffect(() => {
        if (window.innerWidth >= 768 && !activeThreadId && tenantThreads.length > 0) {
            setActiveThreadId(tenantThreads[0].id);
        }
    }, []);
    ...
    return (
        <div className="h-full flex flex-col animate-in fade-in duration-500">
            ...
            <div className="flex-1 flex gap-0 border-2 border-zinc-800 bg-black overflow-hidden shadow-[8px_8px_0px_0px_#000] relative">
                {/* Threads List - Hidden on mobile if activeThreadId is selected */}
                <div className={`w-full md:w-96 border-r-2 border-zinc-800 flex-col bg-zinc-950 ${activeThreadId ? 'hidden md:flex' : 'flex'}`}>
                ...
                {/* Chat Window - Hidden on mobile if no activeThreadId selected */}
                <div className={`flex-1 flex-col bg-zinc-950 relative ${activeThreadId ? 'flex' : 'hidden md:flex'}`}>
                    {activeThread ? (
                        <>
                            ...
                            {/* Header */}
                            <div className="p-4 md:p-6 border-b-2 border-zinc-800 flex justify-between items-center bg-zinc-950 z-10 shadow-md">
                                <div className="flex items-center gap-4">
                                    <button onClick={() => setActiveThreadId(null)} className="md:hidden text-zinc-500 hover:text-white">
                                        <ArrowLeft className="w-5 h-5" />
                                    </button>
                                    <div>
                                        <h3 className="font-black text-white uppercase text-lg md:text-xl tracking-tighter">{activeThread.contactName}</h3>
                            ...
                            {/* Messages */}
                            <div className="flex-1 overflow-y-auto p-4 md:p-10 space-y-8 z-0">
                                {activeThread.messages.map((msg) => (
                                    <div key={msg.id} className={`flex ${msg.direction === 'outbound' ? 'justify-end' : 'justify-start'}`}>
                                        <div className={`max-w-[85%] md:max-w-[65%] group relative flex flex-col ${msg.direction === 'outbound' ? 'items-end' : 'items-start'}`}>
                            ...
                            {/* Input Area */}
                            <div className="p-4 md:p-8 border-t-2 border-zinc-800 bg-zinc-950 z-10">
                                <div className="flex gap-4 items-end">
                                    <button className="hidden md:block p-5 bg-black border-2 border-zinc-800 text-zinc-500 hover:text-white hover:border-zinc-600 transition-all hover:-translate-y-1 hover:shadow-[4px_4px_0px_0px_#000]">
                                    ...
                                    <div className="flex-1 relative">
                                        <textarea 
                                            ...
                                            className="w-full bg-black border-2 border-zinc-800 p-4 md:p-5 text-white text-sm focus:border-lime-400 focus:outline-none resize-none h-14 md:h-20 pr-14 transition-colors font-sans placeholder:text-zinc-700 font-medium"
                                        />
                                        <button 
                                            onClick={handleSmartReply}
                                            disabled={isGenerating}
                                            className="absolute right-2 bottom-2 md:right-4 md:bottom-4 p-2 text-purple-400 hover:text-white transition-colors disabled:opacity-50 bg-purple-900/10 hover:bg-purple-500 border border-purple-500/30 hover:border-purple-500"
                                    ...
                                    <button 
                                        ...
                                        className="p-4 md:p-5 bg-lime-400 text-black font-bold border-2 border-lime-500 hover:translate-y-[2px] hover:translate-x-[2px] hover:shadow-[4px_4px_0px_0px_#000] shadow-[8px_8px_0px_0px_#000] transition-all disabled:opacity-50 disabled:shadow-none disabled:transform-none"
                                    >
